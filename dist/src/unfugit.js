...
let ephemeralCleanupInterval = null;
let sessionState = {
...
// Watcher Instance
let watcher = null;
const ephemeralResources = new Map();
function createEphemeralResource(mimeType, payload, size) {
    ...
    server.registerResource(`ephemeral-${id}`, uri, { title: 'Ephemeral content', description: 'Temporary content registered by unfugit' }, async (u, _extra) => {
...
async function execGitRaw(args, options) {
...
async function gitResolveRef(ref) {
    try {
...
// Get object type
async function gitGetObjectType(ref, filepath) {
    try {
...
// Get git status
async function gitStatus(workdir) {
...
// Count total commits
async function gitCountCommits() {
    try {
...
async function buildNestedTree(files) {
    ...
    async function buildTreeFromStructure(structure) {
        const entries = [];
...
async function gitCreateCommit(files, message) {
    try {
...
async function gitRefExists(ref, options) {
    try {
...
async function gitPathExists(p, ref = 'HEAD', options) {
    try {
        const result = (await execFileAsync('git', ['cat-file', '-t', `${ref}:${p}`], {
            cwd: auditRepoPath,
            ...options,
        }));
        const type = typeof result.stdout === 'string' ? result.stdout.trim() : result.stdout.toString().trim();
...
async function gitCatFile(commitRef, filepath) {
    try {
...
async function gitGetFileMetadata(commit, path) {
    try {
...
async function gitListPaths(commitRef, path = '.') {
    try {
        const commitOid = await git.resolveRef({
            fs: fsFull,
            dir: auditRepoPath,
...
async function getRecentCommits(count, offset = 0, options) {
    try {
...
async function gitLog(args, options) {
    try {
        ...
        for (const commit of commits) {
            ...
            // Filter by options
            if (grep && !commitInfo.message.includes(grep))
                continue;
            if (author && commitInfo.author !== author)
                continue;
            if (since && commitInfo.date < since)
                continue;
            if (until && commitInfo.date > until)
                continue;
...
async function getCommitFileChanges(fromRef, toRef) {
    const result = {
        files: [],
        insertions: 0,
        deletions: 0,
...
async function gitDiff(args) {
    try {
        ...
        // Debug logging
        await sendLoggingMessage('info', `gitDiff: baseOid=${baseOid}, headOid=${headOid}, files=${changes.files.join(',')}`);
        ...
        switch (args.output) {
            ...
            default:
                ...
                if (filteredFiles.length === 0) {
                    await sendLoggingMessage('info', `gitDiff: No files changed between ${args.base} and ${args.head}`);
                    return '';
                }
                for (const file of filteredFiles) {
...
async function calculateRestoreImpact(args) {
    try {
        ...
        const targetFiles = commitFiles.filter((file) => {
            if (args.paths) {
                return args.paths.some((pathspec) => file.startsWith(pathspec.replace(/\*.*$/, '')) || minimatch(file, pathspec));
            }
            return true;
        });
        ...
        await sendLoggingMessage('error', `Failed to calculate restore impact: ${error.message}`);
        throw error;
    }
}
async function gitLogFollow(args) {
    ...
    try {
        ...
        await sendLoggingMessage('error', `Git log --follow failed: ${error.message}`);
        return { path: args.path, commits: [], nextCursor: null };
    }
}
async function generatePatches(commits, args) {
    const patches = [];
    let totalSize = 0;
    for (const commit of commits) {
        try {
            ...
            await sendLoggingMessage('warning', `Failed to generate patch for ${commit.hash}: ${error.message}`);
        }
    }
    patches.totalSize = totalSize;
    return patches;
}
async function gitShow(args) {
    try {
        ...
        const [hash, timestamp, subject, authorName, authorEmail, committerName, committerEmail, parents, message,] = commitOutput.split('\x1e');
        const commit = {
            hash,
            timestamp: new Date(parseInt(timestamp) * 1000).toISOString(),
                    ...
                    try {
                        ...
                        if (sm) {
                            ...
                            if (est > args.max_bytes) {
                                throw new Error(`${DomainErrorCode.SIZE_LIMIT_EXCEEDED}: Estimated patch size ${est} exceeds limit ${args.max_bytes}`);
                            }
                        }
                    }
                    catch (e) {
        ...
        await sendLoggingMessage('error', `Git show failed: ${error.message}`);
        throw error;
    }
}
async function gitLogL(args) {
    const gitArgs = ['log'];
    // Add line range
    if (args.func) {
    ...
    try {
        ...
        await sendLoggingMessage('error', `Git log -L failed: ${error.message}`);
        return { file: args.file, trace: [], nextCursor: null };
    }
}
// --- Gather Complete Stats Implementation ---
async function gatherCompleteStats(detailed) {
...
function hasDomainError(err, code) {
...
function registerAllResources(server) {
    ...
    server.registerResource('audit-repo-info', 'resource://unfugit/repo/info', { title: 'Audit Repo Info', description: 'Repository statistics and state' }, async (uri, _extra) => {
    ...
    server.registerResource('recent-commits', 'resource://unfugit/commits/recent', { title: 'Recent Commits', description: 'Latest audit commits' }, async (uri, _extra) => {
...
function registerAllPrompts(server) {
    server.registerPrompt('unfugit_on_start', {
...
function pathsEqual(paths1, paths2) {
...
async function checkIgnoreCollision(pattern) {
    const existingPatterns = await getCustomIgnores();
    const lowerPattern = pattern.toLowerCase();
    return existingPatterns.some((existing) => existing.toLowerCase() === lowerPattern && existing !== pattern);
}
// --- Session and Role Management ---
function onLeaseAcquired(leaseInfo) {
...
let activeLockFd = null; // File descriptor for exclusive lock
    ...
    try {
        ...
        if (error.code === 'EEXIST' || error.code === 'EACCES') {
            try {
                ...
                const existingLease = JSON.parse(existingContent);
...
async function withWorktreeLock(fn) {
...
// --- Repository Location ---
function getAuditRepoPath(projectPath) {
...
async function performMaintenance(full = false) {
    if (sessionState.role !== 'active') {
        return;
    }
...
class FileWatcher {
    ...
    constructor(projectPath, ignorePatterns, debounceMs = 200) {
    ...
    async start() {
        ...
        this.watcher.on('error', (error) => {
            sendLoggingMessage('error', `Watcher error: ${error.message}`);
        });
    }
    async stop() {
        if (this.watcher) {
    ...
    handleChange(event, filePath) {
        ...
        sendLoggingMessage('debug', `File change detected: ${event} ${relativePath} (pending: ${this.pendingChanges.size})`);
        if (this.debounceTimer) {
            clearTimeout(this.debounceTimer);
        }
    ...
    async processChanges() {
        ...
        try {
            ...
            await sendLoggingMessage('error', `Failed to create audit commit: ${error.message}`);
        }
    }
    async getStats() {
        let maxUserWatches;
        ...
        return {
            ...
            avgCommitLatency: this.stats.commitCount > 0
...
async function getEffectiveIgnores() {
    await loadCustomIgnores();
    return [...DEFAULT_IGNORES, ...customIgnores];
}
async function getCustomIgnores() {
    await loadCustomIgnores();
    return customIgnores;
}
async function manageIgnores(args) {
    await loadCustomIgnores();
    switch (args.mode) {
        case 'check':
            let rules;
        ...
        case 'add': {
            const normalizedPatterns = args.patterns.map((p) => p.replace(/\\/g, '/'));
            const existingSet = new Set(customIgnores);
            const toAdd = normalizedPatterns.filter((p) => !existingSet.has(p));
            const skipped = normalizedPatterns.filter((p) => existingSet.has(p));
            if (!args.dry_run) {
        ...
        case 'remove': {
            const existingSet = new Set(customIgnores);
            const toRemove = (args.patterns || []).filter((p) => existingSet.has(p));
            const notFound = (args.patterns || []).filter((p) => !existingSet.has(p));
            if (!args.dry_run) {
                const toRemoveSet = new Set(toRemove);
                customIgnores = customIgnores.filter((p) => !toRemoveSet.has(p));
                await saveCustomIgnores();
            }
            return {
                mode: 'remove',
...
async function scanProjectFiles(dir, ignorePatterns) {
    const files = [];
    async function scan(currentDir, baseDir) {
        try {
            ...
            for (const entry of entries) {
                ...
                const shouldIgnore = ignorePatterns.some((pattern) => minimatch(relativePath, pattern) || minimatch(fullPath, pattern));
                if (shouldIgnore)
                    continue;
...
async function applyRestore(args) {
    const restored = [];
    const backupPaths = [];
    try {
        ...
        if (args.create_backup_copy) {
            const backupDir = path.join(projectRoot, '.unfugit-backups', new Date().toISOString().split('T')[0]);
        ...
        if (!args.overwrite_unstaged) {
            ...
            if (status === 'dirty') {
                throw new Error(`${DomainErrorCode.DIRTY_WORKTREE}: Worktree has uncommitted changes. Use stash_uncommitted=true or overwrite_unstaged=true`);
            }
        }
        ...
        const filesToRestore = commitFiles.filter((file) => {
            if (previewData.paths) {
                return previewData.paths.some((pathspec) => file.startsWith(pathspec.replace(/\*.*$/, '')) || minimatch(file, pathspec));
            }
            return true;
        });
        ...
        for (const file of filesToRestore) {
            ...
            try {
                ...
                await sendLoggingMessage('warning', `Failed to restore ${file}: ${error.message}`);
            }
        }
        ...
        await sendLoggingMessage('error', `Apply restore failed: ${error.message}`);
        throw error;
    }
}
...
async function checkWorktreeStatus() {
    try {
        ...
        await sendLoggingMessage('error', `Failed to check worktree status: ${error.message}`);
        return 'dirty'; // Assume dirty on error for safety
    }
}
// Create an audit commit
async function createAuditCommit(changedFiles) {
    try {
        const files = new Map();
        const deletedPaths = [];
...
function wrapToolHandler(handler) {
    return async (args, _extra) => {
        try {
            ...
            const domainError = Object.values(DomainErrorCode).find((code) => error.message?.includes(code));
            if (domainError) {
                const errPayload = {
                    code: domainError,
            ...
            // Generic error
            await sendLoggingMessage('error', `Unhandled error: ${error.message}`, {
                stack: error.stack,
            });
...
function registerToolWithErrorHandling(srv, name, config, handler) {
    srv.registerTool(name, config, wrapToolHandler(handler));
}
// --- Notifications ---
async function sendProgressNotification(token, progress, total, message) {
...
async function sendLoggingMessage(level, message, data) {
    // Low-level server API; may throw if called before connect. Swallow pre-connect logs.
    try {
        const payload = typeof data === 'object' && data !== null ? { message, ...data } : { message, _extra: data };
        await server.server.notification({
...
async function sendResourceUpdated(uri) {
...
function onCommitCreated(commit) {
    sessionState.sessionCommits++;
...
function onRoleChange(from, to, epoch) {
    sendLoggingMessage('info', `Role changed from ${from} to ${to}`, { epoch });
    // Notify resource subscribers
    sendResourceUpdated('resource://unfugit/repo/info');
}
// --- Helper for structured content ---
function createToolResponse(content, _structuredContent, text) {
...
function registerAllTools(srv) {
    ...
    registerToolWithErrorHandling(srv, 'unfugit_history', {
        ...
        if (args.merges_only && args.no_merges) {
            throw new McpError(ErrorCode.InvalidParams, 'Cannot specify both merges_only and no_merges');
        }
        let offset = 0;
        if (args.cursor) {
    ...
    registerToolWithErrorHandling(srv, 'unfugit_diff', {
        ...
        const [baseExists, headExists] = await Promise.all([
            gitRefExists(args.base, {
                ...
                abortSignal: _extra?.abortSignal,
            }),
        ...
        if (args.output === 'patch' && diffOutput) {
            ...
            if (exceededCallerLimit || patchBytes > MAX_EMBEDDED_SIZE) {
                const { uri, expiresAt } = createEphemeralResource('text/x-diff', { text: diffOutput }, patchBytes);
                // Provide fetchable URI and separate compact metadata summary
                content.push({ type: 'resource', resource: { uri } });
                content.push({
                    type: 'resource',
                    resource: {
                        ...
                        _meta: {
                            size: Buffer.byteLength(JSON.stringify({ uri, size: patchBytes, mimeType: 'text/x-diff', expiresAt }), 'utf8'),
                        },
                    },
                });
    ...
    // unfugit_show
    registerToolWithErrorHandling(srv, 'unfugit_show', {
        ...
        if (args.output === 'patch' && show.patch) {
            ...
            if (exceededCallerLimit || patchBytes > MAX_EMBEDDED_SIZE) {
                const { uri, expiresAt } = createEphemeralResource('text/x-diff', { text: show.patch }, patchBytes);
                content.push({ type: 'resource', resource: { uri } });
                content.push({
                    type: 'resource',
                    resource: {
    ...
    // unfugit_get
    registerToolWithErrorHandling(srv, 'unfugit_get', {
        ...
        if (contentBuf.length > MAX_EMBEDDED_SIZE) {
            ...
            resourceBlocks.push({ type: 'resource', resource: { uri } });
            resourceBlocks.push({
                type: 'resource',
                resource: {
                    ...
                    _meta: {
                        size: Buffer.byteLength(JSON.stringify({ uri, size: contentBuf.length, mimeType, encoding, expiresAt }), 'utf8'),
                    },
                },
            });
        }
    ...
    registerToolWithErrorHandling(srv, 'unfugit_find_by_content', {
        ...
        ], { commits, term: args.term, count: commits.length }, text);
        if (_extra.progressToken) {
            await sendProgressNotification(_extra.progressToken, 100, 100, 'Done.');
        }
        return resp;
    ...
    registerToolWithErrorHandling(srv, 'unfugit_restore_preview', {
        ...
        if (impact.patch) {
            ...
            if (overLimit || bytes > MAX_EMBEDDED_SIZE) {
                const { uri, expiresAt } = createEphemeralResource('text/x-diff', { text: impact.patch }, bytes);
                content.push({ type: 'resource', resource: { uri } });
                content.push({
                    type: 'resource',
                    resource: {
                        ...
                        _meta: {
                            size: Buffer.byteLength(JSON.stringify({ uri, size: bytes, mimeType: 'text/x-diff', expiresAt }), 'utf8'),
                        },
                    },
                });
            }
    ...
    registerToolWithErrorHandling(srv, 'unfugit_restore_apply', {
    ...
    registerToolWithErrorHandling(srv, 'unfugit_force_commit', {
        title: 'Force Commit',
        ...
        try {
            ...
            const hash = await gitCreateCommit(files, message);
            sessionState.sessionCommits++;
            return createToolResponse([], { hash, filesChanged: files.size }, `Forced commit created: ${hash} (${files.size} files)`);
        }
        catch (error) {
            return createToolResponse([], null, `Failed to force commit: ${error.message}`);
        }
    });
    // unfugit_stats
    registerToolWithErrorHandling(srv, 'unfugit_stats', {
    ...
    // unfugit_ignores
    registerToolWithErrorHandling(srv, 'unfugit_ignores', {
        ...
        if (args.mode === 'add' && args.patterns) {
            ...
            const problematicPatterns = args.patterns.filter((p) => p === '**' || p === '*' || p === '**/*');
    ...
    // unfugit_timeline
    registerToolWithErrorHandling(srv, 'unfugit_timeline', {
        ...
        const overLimit = !!(args.max_bytes && patches.totalSize > args.max_bytes);
        // Decorate commits with change kind and surface created/deleted/renames
        let currentPath = args.path;
        let createdIn = null;
        let deletedIn = null;
        const renameEvents = [];
        async function countLinesAt(commit, p) {
            try {
        ...
        async function computeChangeStats(hash, pathNow, renameFrom) {
            ...
            const magnitude = pct_of_prev === null
                ? 'unknown'
                : pct_of_prev < 5
                    ? 'small'
                    : pct_of_prev < 20
        ...
        if (overLimit) {
            content.push({
                ...
                resource: {
                    ...
                    text: JSON.stringify({
                        code: DomainErrorCode.SIZE_LIMIT_EXCEEDED,
                        totalPatchBytes: patches.totalSize,
        ...
        if (patches.length > 0) {
            patches.forEach((patch) => {
                ...
                if (overLimit || bytes > MAX_EMBEDDED_SIZE) {
                    const { uri, expiresAt } = createEphemeralResource('text/x-diff', { text: patch.content }, bytes);
                    content.push({ type: 'resource', resource: { uri } });
                    content.push({
                        type: 'resource',
                        resource: {
    ...
    // unfugit_trace_lines
    registerToolWithErrorHandling(srv, 'unfugit_trace_lines', {
...
//# sourceMappingURL=unfugit.js.map