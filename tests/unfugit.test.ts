...
class McpClient {
  ...
  private buffer = '';
  private responseMap = new Map<number | string, (response: JsonRpcResponse) => void>();
  private stderr = '';
  private debug = false;
  ...
  async sendRequest(request: JsonRpcRequest): Promise<JsonRpcResponse> {
    return new Promise((resolve, reject) => {
      ...
      if (request.method === 'initialized') {
        ...
        resolve({ jsonrpc: '2.0', id: request.id } as JsonRpcResponse);
        return;
      }
...
describe('unfugit MCP Server', () => {
  ...
  describe('Tool Operations', () => {
    ...
    it('should show diff between commits', async () => {
      ...
      if (commits.length < 2) {
        throw new Error(
          `Expected at least 2 commits, but found ${commits.length}: ${commits.join(', ')}`,
        );
      }
      ...
      let secondCommit: string | null = null;
  ...
  describe('Resources', () => {
    ...
    it('should list available resources', async () => {
      ...
      while (attempts < maxAttempts) {
        ...
        resourceUris = resourcesResponse.result.resources.map((r: any) => r.uri);
        ...
        if (attempts < maxAttempts) {
          console.warn(
            `Attempt ${attempts}: unfugit://stats not found in resources:`,
            resourceUris,
          );
